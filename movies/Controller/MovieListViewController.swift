...
class MovieListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchControllerDelegate, UISearchResultsUpdating {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if searching {
            ...
            let aux = MovieListViewController.cacherImage.object(forKey:  ("\(searchMovie[indexPath.item].id ?? -1)" as AnyObject) as! NSString)
        ...
        if !Reachability.isConnectedToNetwork() {
            ...
            if searching {
                ...
                let aux = MovieListViewController.cacherImage.object(forKey:  ("\(movies[indexPath.item].id ?? -1)" as AnyObject) as! NSString)
                let aux2 = aux?.cacheImage != nil ? aux?.cacheImage : UIImage(imageLiteralResourceName: "No_image_available.svg")
                cell.pictureImageView.image = aux2
            }
        ...
        if !searching {
            if let imageURL = URL(string: "https://image.tmdb.org/t/p/w500\(movies[indexPath.item].image ?? "")") {
                ...
                DispatchQueue.global().async {
                    ...
                    MovieListViewController.cacherImage.setObject(cacheImage, forKey: "\( self.movies[indexPath.item].id ?? -1)" as NSString)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let infoController = MovieViewController()
        let currentCell = tableView.cellForRow(at: indexPath) as! MovieCell
                ...
                do {
                    ...
                    for result in results {
                        ...
                        if self.type == Type.MOVIES.rawValue {
                            ...
                            let movie = Movie(id:  (result["id"] as! Int), image: (imageString), title: (result["original_title"] as! String), releaseDate: (result["release_date"] as! String), overview: (result["overview"] as! String), popular: popular, topRated: topRated, upcoming: upcoming, type: self.type)
                            
                            self.movies.append(movie)
                            
                            let cacheInfo = Cache()
                            cacheInfo.cacheInfo = movie
                            MovieListViewController.cacher.setObject(cacheInfo, forKey: "\(movie.id ?? -1)" as NSString)
                        }
                        else {
                            
                            let imageString = result["poster_path"] as? String ?? ""
                            
                            let movie = Movie(id: (result["id"] as! Int), image: (imageString ), title: (result["original_name"] as! String), releaseDate: (result["first_air_date"] as! String), overview: (result["overview"] as! String), popular: popular, topRated: topRated, upcoming: upcoming, type: self.type)
    ...
    func loadCachedInfo() {
        ...
        if let all =  MovieListViewController.cacher.value(forKey: "allObjects") as? NSArray {
            
            for object in all {
                
                let cachedObject = object as! Cache
                let cachedInfo = cachedObject.cacheInfo as Movie
                
                if(category == Category.POPULAR.rawValue && cachedInfo.popular == true && cachedInfo.type == type) {
                    let movie = Movie(id: cachedInfo.id!, image: cachedInfo.image!, title: cachedInfo.title!, releaseDate: cachedInfo.releaseDate!, overview: cachedInfo.overview!, popular: cachedInfo.popular!, topRated: cachedInfo.topRated!, upcoming: cachedInfo.upcoming!, type: type)
                    movies.append(movie)
                }
